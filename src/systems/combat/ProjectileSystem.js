...
import { spawnCoin, spawnParticles, floaty, spawnLocLoot } from '../world/SpawnSystem.js';
...
export function updateArrows(dt) {
  ...
  for (let i = arrows.length - 1; i >= 0; i--) {
    ...
    if (ar.hitKind === "enemy") {
      for (const e of enemies) {
        ...
        if (dist(ar.x, e.x) < et.w * 0.7 && Math.abs(ar.y - enemyDrawY) < (et.dragon ? 85 : 40)) {
          ...
          floaty(e.x, (crit.isCrit ? "⭐ " : "") + "-" + crit.damage, crit.isCrit ? "#ffff00" : "#8a2a4a", crit.isCrit ? 24 : 15);
    ...
    if (!hit && ar.hitKind === "base" && (ar.y > groundY - 8 || ar.life <= 0)) {
      ...
      if (dist(ar.x, player.x) < r * 0.6 && player.jumpH <= 0 && player.invuln <= 0 && !window._DEV_GOD_MODE) {
